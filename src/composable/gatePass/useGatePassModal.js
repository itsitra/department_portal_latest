...
import { ref } from "vue";
import gatePassTable from "./useGatePassTable";
import gatePassPaginate from "../gatePass/useGatePassPaginate";
...
const remarks = ref(
  sessionStorage.getItem("depCode") === "10" ? "Instruments send Through" : ""
);
...
const showEditModal = (e) => {
  ...
  returndate.value = a[0].returndate;
  ...
  if (a[0].typeuser === "customer") {
    getCustData({ name: a[0].name, id: a[0].custid });
  }
...
const getCustData = (q) => {
  ...
  if (typeof q !== "object") {
      ...
      .then((res) => {
        if (res.status == 200) {
          if (res.data === "false") {
            ...
            if (edit.value) {
              custSelected.value = cust.value[0];
            }
            queryLoading.value = false;
          }
          queryLoading.value = false;
    ...
    axios
      .get(
        ` http://192.168.1.9/departmentportalapitestreceipt/index.php/api/limsCustomer?id=${q.id}`
      )
      .then((res) => {
        if (res.status == 200) {
          if (res.data === "false") {
