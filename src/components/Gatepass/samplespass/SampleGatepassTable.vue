      ...
      <table class="table w-full table-compact table-zebra">
        <thead>
          <tr class="bg-gray-300 text-gray-700 font-extrabold">
            <th>GatePass Id</th>
            <th v-if="depCode === '4'">Department</th>
            <th>{{ tableHeader1 }}</th>
            <th>{{ tableHeader5 }}</th>
            <th>{{ tableHeader2 }}</th>
            ...
            <td class="dropdown dropdown-left items-center flex justify-center">
              ...
              <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box gap-3">
                ...
                ">
                  <router-link :to="{
                    name: 'pdf',
                    params: { id: gatePassData[i].rgpId },
                  }" target="_blank">
...
<script>
import gatePassTable from "../../../composable/gatePass/samplepass/useSampleGatePassTable";
import gatePassModal from "../../../composable/gatePass/samplepass/useSampleGatePassModal";
import gatePassPaginate from "../../../composable/gatePass/samplepass/useSampleGatePassPaginate";
import gatePassLine from "./SampleGatePassline.vue";
import GatePassView from "./SampleGatePassView.vue";
...
export default {
  ...
  setup() {
    ...
    const { pages, pageno, loading, gatePassData, getData } =
      gatePassPaginate();
    return {
      view,
      viewI,
