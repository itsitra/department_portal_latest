...
export default {
  ...
  setup(props) {
    ...
    const saveData = () => {
      if (gatePass.value !== "" && staffName.value !== "Staff Name") {
        if (lineEntries.value.length !== 0) {
          if (
            staffName.value.length !== 0 &&
            (supplierName.value.length !== 0 || custSelected.value !== null)
          ) {
            if (gatePass.value === "Returnable" && returndate.value === "") {
              alert("Select Approximate Return Date");
            } else {
              let data = {
                ...
                supplierCode: userCheck.value
                  ? null
                  : Number(supplierName.value),
                ...
                transportCompany:
                  transportCompany.value.length == 0
                    ? document.getElementsByName("transportCompany")[0]
                        .placeholder
                    : transportCompany.value,
                vehicleNo:
        ...
        alert("Select Gate Pass Type And Staff Name");
      }
    };
